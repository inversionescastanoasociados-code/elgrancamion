...
export default function NumerosDisponiblesPage() {
  ...
  const paddedPageNumbers = useMemo(() => {
    const slots: (number | null)[] = [...currentNumbers];
    ...
    <div className="min-h-screen bg-white text-[#1A1A1A] overflow-x-hidden">
      ...
      <section className="max-w-[1400px] mx-auto px-3 sm:px-6 py-2">
        {loading ? (
          ...
          <>
            {/* Grid — 11×15 fijo; celdas vacías en última página evitan estirar */}
            <div
            ...
            >
              {paddedPageNumbers.map((num, idx) => {
                if (num === null) {
                  return <div key={`empty-${pageIndex}-${idx}`} className="h-full w-full" aria-hidden />;
                }
